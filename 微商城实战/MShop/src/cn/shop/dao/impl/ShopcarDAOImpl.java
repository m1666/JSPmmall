...
import cn.shop.dao.IShopcarDAO;
import cn.shop.vo.Shopcar;
...
public class ShopcarDAOImpl extends AbstractDAOImpl implements IShopcarDAO {
    ...
    public boolean doUpdateAmount(String mid,Integer gid) throws SQLException {
        String sql = new String("UPDATE shopcar SET amount=amount+1 WHERE mid=? AND gid=?") ;
    ...
    @Override
    public boolean doCreatBatch(Set<Shopcar> vos) throws SQLException {
        boolean flag = true ;
        String sql = "INSERT INTO shopcar(gid,mid,amount) VALUES(?,?,?)" ;
        super.pstmt = super.conn.prepareStatement(sql) ;
        Iterator<Shopcar> iterator = vos.iterator() ;
        while (iterator.hasNext()) {
            Shopcar vo = iterator.next() ;
